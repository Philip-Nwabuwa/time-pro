...
interface PageImagePickerProps {
  initialImageUrl?: string | null;
  onImageChange?: (croppedBlob: Blob | null) => void;
  aspectRatio?: number; // Default to 16:9 for page banners
...
export default function PageImagePicker({
  ...
  onImageChange,
  aspectRatio = 16 / 9, // 16:9 aspect ratio for page banners
  maxSize = 5 * 1024 * 1024, // 5MB default
  className = "",
}: PageImagePickerProps = {}) {
  const [
    { files, isDragging },
    {
      handleDragEnter,
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // 4. Set the final image state to the NEW URL
      setFinalImageUrl(newFinalUrl);

      // 5. Notify parent component about the cropped blob
  ...
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
        ...
        {finalImageUrl ? (
          ...
          <div className="relative group">
            ...
            <Button
              ...
              size="icon"
              className="absolute top-2 right-2 size-8 rounded-full bg-black/50 hover:bg-black/70 text-white opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              className="absolute bottom-2 right-2 bg-white/90 hover:bg-white opacity-0 group-hover:opacity-100 transition-opacity"
            >
              Change Image
            </Button>
          </div>
          ...
          <button
            type="button"
            className="w-full aspect-video border-2 border-dashed border-gray-300 hover:border-gray-400 rounded-lg flex flex-col items-center justify-center p-8 text-gray-500 hover:text-gray-600 transition-colors bg-gray-50 hover:bg-gray-100 data-[dragging=true]:border-blue-400 data-[dragging=true]:bg-blue-50"
            onClick={handleFileDialogClick}
          ...
          >
            ...
            <p className="text-sm font-medium mb-1">
              Click to upload a page banner
            </p>
            <p className="text-xs text-gray-400">
              Max {Math.round(maxSize / (1024 * 1024))}MB • JPG, PNG, GIF • 16:9
              recommended
            </p>
          </button>
        )}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen} modal={false}>
        <DialogContent className="gap-0 p-0 sm:max-w-4xl *:[button]:hidden">
          <DialogDescription className="sr-only">
            Crop page image dialog
          </DialogDescription>
